  ...
  module Smells
    ...
    class SmellRepository
      ...
      def initialize(smell_types: self.class.smell_types,
        ...
        @detectors     = smell_types.map do |klass|
          { klass => klass.new(configuration_for(klass)) }
        end.reduce({}, :merge)
      end

      def report_on(collector)
        detectors.each_value { |detector| detector.report_on(collector) }
      end
      ...
      def smell_detectors_for(type)
        @smell_detectors_for ||= Hash.new { |hash, key| hash[key] = [] }.tap do |hooks|
          detectors.each_value { |detector| detector.register(hooks) }
        end
        @smell_detectors_for[type]
      end
    end
  end
end
